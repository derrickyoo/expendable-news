...
<body>
		...
		<table>
			<tr>
				...
				<td id='main-post-category'>
					...
					?>
					<img src="images/folder_open_icon&amp;16.png" width="10" height="10" alt="Folder Open Icon&amp;16"> <a href='category.php?cat=<?= $_SESSION['category'] ?>'><?= $_SESSION['category'] ?></a>
				</td>
				<td id='main-login'>
					<?php
						if (!isset($_SESSION['username']) || empty($_SESSION['username'])) {
	...
	<?php
		...
		$count_categories_query = $db->prepare("SELECT COUNT(`post_id`) FROM `posts` WHERE `category`=:category");
		$params = array(
			':category' => $category
		);
